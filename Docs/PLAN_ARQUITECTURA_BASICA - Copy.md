...
2. Vista General (Index - Acceso Público)
- Acceso sin login obligatorio
- Mostrar contratistas destacados con sus trabajos
- Mostrar últimas reseñas de la comunidad
- Mostrar información de planes disponibles (teaser)
- Visitantes NO AUTENTICADOS: pueden ver perfiles y galerías de trabajos, pero NO pueden contactar
- Visitantes AUTENTICADOS: acceso completo (contactar, dejar reseñas, etc.)

3. Contratistas
- Listado de perfiles con filtros por especialidad y zona
- Vista de perfil público con galería de trabajos
- **Sistema de búsqueda fuzzy (tolerante a errores)**:
  * Búsqueda inteligente que encuentra coincidencias incluso si el cliente escribe diferente
  * Ejemplo: "soldadura", "SOLDADURA", "soladura" todos encuentran contratistas de soldadura
  * Busca en: especialidad, servicios, nombre del contratista y zona
  * Algoritmo: Levenshtein Distance + matching parcial
  * Resultados ordenados por relevancia (similitud)
- Galería de trabajos visible para todos (autenticados y no autenticados)
- Sistema de contacto diferenciado por rol:
  * **Visitantes no autenticados**: ven teléfono (informativo) + enlace para iniciar sesión
  * **Clientes autenticados**: 3 opciones de contacto:
    - Botón WhatsApp (abre chat directo con mensaje personalizado)
    - Botón Llamar (llamada directa)
    - Botón "Solicitar servicio" (formulario formal + notificación al contratista)
  * **Contratistas/Admin**: botón estándar de contacto
- Búsqueda disponible con tecla Enter o botón "Aplicar filtros"
...
- Clientes autificados pueden dejar reseñas (1-5 estrellas + comentario)
- Mínimo 30 caracteres, máximo 500 caracteres en comentario
- Selector visual de estrellas con hover effects  
- Las reseñas se muestran en tarjeta del contratista (últimas 3)
- Rating promedio se calcula automáticamente
- Total de reseñas se actualiza en tiempo real
- Modal elegante con validación de datos
